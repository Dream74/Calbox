...
def core( mda, user, code, question):
  ...
  if com_msn == "" :
    ...
    if style_msn == "" :
     ...
     return json_message( style_msn, STYPLE_ERR)
  else :
...
def get_code( mda ):
  ...
  if not os.path.exists( code_dir ) or not os.path.exists( code_dir + FILE_NAME  + FE_C ):
    return ""
  
  code_file = open(  code_dir + FILE_NAME  + FE_C , 'r' )
